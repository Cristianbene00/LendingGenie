...
import {
  MessageSquareText, Send, Bot, ThumbsUp, ThumbsDown, ChevronDown,
  ChevronRight, Sparkles, Shield, TrendingUp, CreditCard, Home as HomeIcon, Briefcase,
  Car, AlertCircle, CheckCircle2, HelpCircle, ExternalLink,
} from 'lucide-react';
import { Button, Badge, Card, Textarea, Spinner, Toaster, toast, cn } from './ui';
...
function ChatView() {
  ...
  async function send(q?: string) {
    ...
    try {
      const d = await apiPost<{
        queryId: string; answer: string; confidence: number;
        sufficientContext: boolean; escalation?: boolean;
        citations: { qaId: string; question: string; similarity: number }[];
      }>('/api/ask', { question: text });
      setMessages((m) => [...m, {
        id: newId(), role: 'assistant', text: d.answer,
        queryId: d.queryId, confidence: d.confidence,
        sufficientContext: d.sufficientContext, escalation: d.escalation,
  ...
  function clearChat() {
    setMessages([{ id: newId(), role: 'assistant', text: 'New conversation started. How can I help with your credit or loan questions?' }]);
  }

  return (
    <div className="mx-auto flex w-full max-w-3xl flex-1 flex-col px-4 py-6">
      {/* Hero text — only shown when chat is blank */}
      {isBlank && (
        <div className="mb-6 text-center">
          <h1 className="text-2xl font-bold text-slate-900">
            Understand your credit.<br />Find the right loan.
          </h1>
          <p className="mt-2 text-sm text-slate-500">
            Ask me anything about your credit situation, score, or loan options.
          </p>
        </div>
      )}
      ...
      <Card className="flex flex-1 flex-col overflow-hidden" style={{ minHeight: isBlank ? 'auto' : '520px', maxHeight: '680px' }}>
        ...
        <div className="border-t border-slate-100 p-3">
          <div className="flex items-end gap-2">
            ...
            <Button onClick={() => send()} loading={loading} className="h-[42px] px-3">
  ...
  return (
    <div className="flex gap-2.5">
      ...
      <div className="max-w-[85%] min-w-0 flex-1">
        <div className="whitespace-pre-wrap rounded-2xl rounded-tl-sm bg-white px-4 py-2.5 text-sm leading-relaxed text-slate-800 ring-1 ring-slate-200 shadow-sm">
          {msg.text}
        </div>
        ...
        {msg.queryId && (
          <div className="mt-1.5 flex flex-wrap items-center gap-2 pl-1 text-xs text-slate-400">
            ...
            <button onClick={() => handleRate(1)}
              ...
              title="Helpful"><ThumbsUp className="h-3.5 w-3.5" /></button>
            <button onClick={() => handleRate(-1)}
              className={cn('rounded p-1 transition-colors hover:bg-slate-100', msg.rated === -1 && 'text-rose-600')}
              title="Not helpful"><ThumbsDown className="h-3.5 w-3.5" /></button>
...
const STATIC_FAQS: QA[] = [
  {
    ...
    answer: 'A credit score is a number (typically 300–850) that summarizes how reliably you have repaid debt in the past. Lenders use it to decide whether to approve your loan and what interest rate to offer. Higher scores mean better loan terms.',
    category: 'Credit Basics',
  },
  {
    id: 's2',
    question: 'What factors make up my credit score?',
    answer: 'Your FICO score is calculated from five factors: payment history (35%), amounts owed (30%), length of credit history (15%), new credit (10%), and credit mix (10%). Payment history and utilization have the biggest impact.',
    category: 'Credit Basics',
  },
  {
    id: 's3',
    question: 'How can I improve my credit score quickly?',
    answer: 'The fastest wins are paying down credit card balances to reduce your utilization ratio, making sure all accounts are current (no missed payments), and disputing any errors on your credit report. Avoid opening new accounts unnecessarily.',
    category: 'Credit Improvement',
  },
  {
    id: 's4',
    question: 'What credit score do I need to get a personal loan?',
    answer: 'Most traditional lenders prefer a score of 670 or above for personal loans. Some lenders work with scores as low as 580, though you may face higher interest rates. A score above 750 typically gets the best rates available.',
    category: 'Loan Eligibility',
  },
  {
    id: 's5',
    question: 'What is a debt-to-income ratio?',
    answer: 'Your debt-to-income (DTI) ratio is your total monthly debt payments divided by your gross monthly income. Lenders use it to gauge affordability. Most prefer a DTI below 43%, and the lower the better for getting approved.',
    category: 'Loan Eligibility',
  },
  {
    id: 's6',
    question: 'Does applying for a loan hurt my credit score?',
    answer: 'Submitting a formal loan application triggers a hard inquiry, which can temporarily lower your score by a few points. Rate shopping with multiple lenders within a 14-45 day window usually counts as a single inquiry.',
    category: 'Credit Basics',
  },
];
...
function FaqView() {
  ...
  const grouped = items.reduce<Record<string, QA[]>>((acc, q) => {
            ...
            <div className="divide-y divide-slate-100 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
              {qs.map((q) => (
                <div key={q.id}>
                  ...
                  {open === q.id && (
                    <div className="animate-fade-in border-t border-slate-100 bg-slate-50 px-5 py-4 text-sm leading-relaxed text-slate-600">
                      {q.answer}
                    </div>
                  )}
                </div>
...
type LoanOffer = {
  ...
  minScore: number;
  badge?: string;
  badgeColor?: 'green' | 'blue' | 'amber' | 'violet';
  cta: string;
};
...
const LOAN_OFFERS: LoanOffer[] = [
  {
    ...
    icon: CreditCard,
    name: 'Personal Loan',
    description: 'Flexible funds for debt consolidation, medical bills, home improvements, or any personal expense.',
    amount: 'Up to $50,000',
    rate: 'From 7.99% APR',
    term: '12–84 months',
    minScore: 620,
  ...
  },
  {
    ...
    description: 'Build or repair your credit history with a small secured loan designed for all credit types.',
    amount: '$500–$2,000',
    rate: 'From 10.99% APR',
    term: '12–24 months',
    minScore: 0,
  ...
  },
  {
    ...
    description: 'Tap into your home equity for large expenses at lower rates than unsecured borrowing.',
  ...
  },
  {
    ...
    icon: Car,
    name: 'Auto Loan',
    description: 'Finance a new or used vehicle with competitive rates and fast approval decisions.',
    amount: 'Up to $75,000',
    rate: 'From 5.99% APR',
    term: '24–84 months',
    minScore: 600,
  ...
  },
  {
    ...
    icon: Briefcase,
    name: 'Business Loan',
    description: 'Fund your business growth — equipment, working capital, or expansion.',
  ...
  return (
    <div className="mx-auto w-full max-w-5xl px-4 py-10">
      <div className="mb-8 text-center">
        ...
        <p className="mt-2 text-sm text-slate-500">
          Find a loan that matches your credit profile. Use the chat to understand your eligibility first.
        </p>
      </div>
          ...
          return (
            <Card key={offer.id} className="flex flex-col p-5">
              <div className="flex items-start justify-between">
                ...
                {offer.badge && <Badge color={offer.badgeColor ?? 'gray'}>{offer.badge}</Badge>}
              </div>
              ...
              <div className="mt-4 space-y-1.5 rounded-lg bg-slate-50 p-3 text-xs">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-slate-800">{offer.amount}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-500">Rate</span>
                  <span className="font-medium text-slate-800">{offer.rate}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-500">Term</span>
                  <span className="font-medium text-slate-800">{offer.term}</span>
                </div>
                {offer.minScore > 0 && (
                  <div className="flex justify-between">
                    <span className="text-slate-500">Min. score</span>
                    <span className="font-medium text-slate-800">{offer.minScore}+</span>
                  </div>
                )}
              </div>

              <Button className="mt-4 w-full justify-center" variant="outline">
                {offer.cta}
      ...
      <div className="mt-10 flex items-start gap-3 rounded-xl border border-amber-100 bg-amber-50 p-4 text-xs text-amber-800">
        ...
        <p>
          Rates and amounts shown are illustrative and subject to credit approval. Actual offers depend on your credit profile and other factors. Use the chat assistant to better understand your eligibility before applying.
        </p>
      </div>
    </div>
  );
